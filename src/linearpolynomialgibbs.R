...
ratsGibbs <- function(nt) {
  ...
  for (t in 1:nt) {
    ...
    meanprop <- (N*T*(beta*xbar-gamma*x2bar-gamma)*alphaprior[2] + alphaprior[1]*param[4])/(N*T*alphaprior[2]+param[4])
    varprop <- param[4]*alphaprior[2]/(N*T*alphaprior[2]+param[4])
    
    param[1] <- rnorm(1,meanprop,sqrt(varprop))
    
    # Update beta (from Normal)
    
    meanprop <- (N*T*(alpha*xbar+x2bar+gamma*xbar*x2bar-xbar*ybar)*betaprior[2]*param[4])/(N*T*betaprior[2]+param[4])
    varprop <- param[4]*betaprior[2]/(N*T*betaprior[2]+param[4])
    
    param[2] <- rnorm(1,meanprop,sqrt(varprop))
    
    # Update gamma (from Normal)
    
    meanprop <- (N*T*(alpha*x2bar+beta*xbar*x2bar+x2bar^2-x2bar*ybar)*gammaprior[2]*param[4])/(N*T*gammaprior[2]+param[4])
    varprop <- param[4]*gammaprior[2]/(N*T*gammaprior[2]+param[4])
    
    param[3] <- rnorm(1,meanprop,sqrt(varprop))
    ...
    for (i in 1:N) {
      for (j in 1:T) {
        sumgamma <- sumgamma + (data[i,j]-param[1]-param[2]*(x[j])-param[3]*x[j]^2)^2	
